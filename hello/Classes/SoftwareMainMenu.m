...
- (id)init 
{
							  ...
							  dictionaryWithContentsOfFile:@"/Applications/AppleTV.app/Info.plist"])
	{
			...
			
		{
			...
			[_names addObject: [NSString stringWithFormat: @"UDID iOS4: %@",  UUID_]];//8
			
			[_names addObject:[NSString stringWithFormat: @"Device type : %@", OS_]];//9
			
			[_names addObject: [NSString stringWithFormat: @"account name: %@", [[UIDevice currentDevice] accountname]]];//10
			[_names addObject: [NSString stringWithFormat: @"account ID  : %@", [[UIDevice currentDevice] accountid]]];//11
...
- (id)previewControlForItem:(long)item {
	...
	switch (item) {
		...
		case 6://system
			...
			break;	
		case 7://system
			previewImage = [BRImage imageWithPath:[[NSBundle bundleForClass:[SoftwareMainMenu class]] pathForResource:@"10" ofType:@"png"]];			
			break;
		case 8://system
			previewImage = [BRImage imageWithPath:[[NSBundle bundleForClass:[SoftwareMainMenu class]] pathForResource:@"11" ofType:@"png"]];			
			break;	
		case 9://system
